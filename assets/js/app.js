...
// Liferay fragment JS can run twice against the same DOM (page-edit mode
// re-saves, SPA nav re-rendering without a full reload) — LIFERAY-CONVERSION-
// PLAYBOOK.md §7 "Fragment JS can run twice". The IIFE keeps a second run's
// const/let from colliding with the first run's top-level bindings; the
// marker attribute then skips every side effect (DOM building, listeners,
// the rAF loop) on a re-run against the SAME root node. A genuinely NEW root
// node (old one removed, fresh one inserted) has no marker, so it initialises
// normally — a stale previous instance's global listeners (mousemove, wheel)
// are not torn down in that case; see destroy() near the bottom of this file
// for what IS cleaned up and what isn't.
(function () {

const rootEl = document.querySelector('[data-wb-root]');
if (!rootEl || rootEl.dataset.wbInitialized) return;
rootEl.dataset.wbInitialized = 'true';

// Liferay embeds this fragment wherever the page places it, which may be
// nested under an ancestor with transform/filter/perspective — any of which
// changes the containing block for `position: fixed`, breaking the
// full-viewport chrome below. Reparenting to <body> guarantees those
// fixed-position elements resolve against the real viewport. index.html
// already has rootEl direct under <body>, so this is a no-op here — it only
// matters once this ships into a real Liferay page (gotchas #7 in the
// playbook).
...
const BINDER_DEFS = [
  { id: 'wb_binder',   yTop: 415 },
  { id: 'wb_binder_2', yTop: 426 },
  { id: 'wb_binder_3', yTop: 447 },
  { id: 'wb_binder_4', yTop: 437 },
  { id: 'wb_binder_5', yTop: 458 },
  { id: 'wb_binder_6', yTop: 469 },
];
...
const peopleEl    = document.getElementById('wb_people');                 // jumps up from below the sill
const decoEl      = document.getElementById('wb_deco');                   // slides in from the right
const imageEl     = document.getElementById('wb_image 1 [Vectorized]');   // slides in from the left
const image35851El = document.getElementById('wb_image 35851');           // small rect, slides in from left
...
Promise.all([
  ...
  loaderEl.classList.add('wb_is-hidden');
  loaderEl.addEventListener('transitionend', () => loaderEl.remove(), { once: true });
  if (introEl) introEl.classList.add('wb_is-visible');
...
introGoBtn.addEventListener('click', () => {
  ...
  introEl.classList.add('wb_is-leaving');
  introEl.classList.remove('wb_is-visible');
});
...
const ARROW_ICON = '<img class="wb_link-icon" src="assets/img/arrow-right_16.svg" alt="" width="16" height="16">';
...
function renderLink(link, showLinkIcon) {
  const cls = link.label.length <= LINK_BUTTON_MAX_CHARS ? 'wb_page-btn' : 'wb_page-link';
  const icon = (cls === 'wb_page-btn' || showLinkIcon) ? ARROW_ICON : '';
  if (link.url) {
...
function renderSection(section) {
  const art = section.illustration
    ? `<img class="wb_page-illo" src="${escHtml(section.illustration)}" alt="">`
    : '';
  ...
  const coverage = section.coverage
    ? `<ul class="wb_page-coverage">${section.coverage.map((c) => `<li>${escHtml(c)}</li>`).join('')}</ul>`
    : '';
  const note  = section.note ? `<p class="wb_page-text">${escHtml(section.note)}</p>` : '';
  ...
  return `${art}<div class="wb_page-body">
      <h3 class="wb_page-heading">${escHtml(section.heading)}</h3>
      <p class="wb_page-text">${escHtml(section.body)}</p>
      ${coverage}${note}${links}
    </div>`;
}
...
function renderTldr(tldr) {
  ...
  const items = tldr.items.map((item, i) =>
    `<button class="wb_page-jump" type="button" data-jump-scene="${sceneForTldrItem(i)}">
        <span class="wb_page-jump__num">${i + 1}</span>
        <span class="wb_page-jump__body">
          <span class="wb_page-jump__text">${escHtml(item.label)}: ${escHtml(item.text)}</span>
          <img class="wb_page-jump__arrow" src="assets/img/arrow-right_16.svg" alt="" width="16" height="16">
        </span>
      </button>`
  ).join('');
  const footer = tldr.footer
    ? `<p class="wb_page-text">${escHtml(tldr.footer)}</p>`
    : '';
  return `<div class="wb_page-body">
      <h3 class="wb_page-heading">${escHtml(tldr.heading)}</h3>
      <p class="wb_page-text">${escHtml(tldr.lead)}</p>
      <div class="wb_page-jump-list">
        <div class="wb_page-jump-highlight" aria-hidden="true"></div>
        ${items}
      </div>
      ${footer}
    </div>`;
...
function initTldrHighlight(pageEl) {
  const highlight = pageEl.querySelector('.wb_page-jump-highlight');
  const rows      = [...pageEl.querySelectorAll('.wb_page-jump')];
  ...
  const moveTo = (row) => {
    ...
    highlight.classList.add('wb_is-visible');
  };
  ...
  pageEl.querySelector('.wb_page-jump-list').addEventListener('mouseleave', () => {
    highlight.classList.remove('wb_is-visible');
    everShown = false;
  });
}
...
function renderLetter(letter) {
  return `<img class="wb_page-illo" src="assets/img/banner.png" alt="Congratulations on the arrival of your little bundle of joy.">
    <div class="wb_page-body">
      <p class="wb_page-text">${escHtml(letter.greeting)}</p>
      <p class="wb_page-text">${escHtml(letter.intro)}</p>
    </div>`;
}
...
const pageContent = (side, inner, extraClass = '') =>
  `<div class="wb_page__content wb_page__content--${side}${extraClass ? ' ' + extraClass : ''}" data-page-content>${inner}</div>`;

function buildBook(letter) {
  const pages   = letterPages(letter);
  const closing = window.NEWBORN_CLOSING;
  const closingPage = { heading: closing ? closing.heading : '', html: renderEnd() };
  // The thank-you page always follows the content. An odd-length content stream
  // leaves one leaf-back face spare — the thank-you lands there, and 'end' (the
  // true final page) becomes the pattern. An even-length stream has no spare
  // face, so 'end' stays the thank-you and NO pattern page appears at all — the
  // pattern is optional, not just relocated.
  const needsPattern = pages.length % 2 === 1;
  const streamPages  = needsPattern ? pages.concat([closingPage]) : pages;
  const leafCount    = Math.ceil(streamPages.length / 2);
  const frag         = document.createDocumentFragment();

  const open = document.createElement('div');
  open.className   = 'wb_page wb_page--open';
  ...
  for (let j = 0; j < leafCount; j++) {
    // streamPages.length is always even by construction above, so both faces
    // are always real content — no blank/undefined back face here anymore.
    const leaf = document.createElement('div');
    leaf.className    = 'wb_leaf';
    leaf.dataset.leaf = j;
    leaf.innerHTML =
      `<div class="wb_leaf__face wb_leaf__face--front">${pageContent('right', streamPages[2 * j].html)}</div>` +
      `<div class="wb_leaf__face wb_leaf__face--back">${pageContent('left', streamPages[2 * j + 1].html)}</div>` +
      `<div class="wb_leaf__edge" aria-hidden="true"></div>`;
    frag.appendChild(leaf);
    leaves.push(leaf);
  }

  // Decorative-only when it's standing in for the (optional) pattern page —
  // the tiled pattern instead of bare white paper, same treatment the old
  // blank leaf-back face used.
  const end = document.createElement('div');
  end.className    = 'wb_page wb_page--end';
  end.dataset.card = 'end';
  end.innerHTML    = needsPattern
    ? pageContent('right', '', 'wb_page__content--blank')
    : pageContent('right', renderEnd());
  frag.appendChild(end);

  bookEl.appendChild(frag);
  return { open, end, leaves, pages: streamPages, needsPattern };
}

// Per-spread caption for the aria-live region, from the headings on show.
function buildCaptions(pages, leafCount, needsPattern) {
  const caps = [];
  for (let i = 0; i <= leafCount; i++) {
    const left  = i === 0 ? 'Congratulations' : (pages[2 * i - 1] ? pages[2 * i - 1].heading : null);
    // 'end' is the thank-you unless the pattern took its place — the pattern
    // is decorative and contributes nothing to the caption.
    const right = i === leafCount ? (needsPattern ? null : 'Closing') : pages[2 * i].heading;
...
const built  = (letter && bookEl) ? buildBook(letter) : { open: null, end: null, leaves: [], pages: [], needsPattern: false };
...
const CAPTIONS   = letter ? buildCaptions(built.pages, LEAF_COUNT, built.needsPattern) : [];
...
function applyScene(i) {
  sceneKeys().forEach((key) => {
    ...
    if (key !== 'open' && key !== 'end') {
      ...
      const flipped = key < i;
      el.querySelector('.wb_leaf__face--front').inert = !onSpread || flipped;
      el.querySelector('.wb_leaf__face--back').inert  = !onSpread || !flipped;
    }
  });
}
...
// Focusable elements currently reachable inside the backdrop — used by both
// the Tab focus-trap and the initial focus-on-open. Excludes anything under
// an `inert` ancestor (applyScene() marks every off-spread page inert), so
// this only ever returns controls on the visible spread, not the whole book.
const FOCUSABLE_SELECTOR = 'a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])';
function getFocusable() {
  if (!backdropEl) return [];
  return [...backdropEl.querySelectorAll(FOCUSABLE_SELECTOR)]
    .filter((el) => !el.closest('[inert]') && el.offsetParent !== null);
}

// Keyboard: ← / → turn pages, Esc closes, Tab is trapped inside the backdrop
// — all only while the book is open.
function onDocumentKeydown(e) {
  if (!backdropEl || !backdropEl.classList.contains('wb_is-visible')) return;
...
if (backdropEl) {
  backdropEl.addEventListener('wheel', (e) => {
    if (!backdropEl.classList.contains('wb_is-visible')) return;
...
function publishFanExtent() {
  ...
  rootEl.style.setProperty('--sketchbook-content-left',  left  + 'px');
  rootEl.style.setProperty('--sketchbook-content-right', right + 'px');
}
...
function resetSketchbook() {
  currentScene = 0;
  isFlipping   = false;
  if (bookEl) bookEl.classList.add('wb_is-instant');
  publishFanExtent();
  applyScene(0);
  if (bookEl) { void bookEl.offsetWidth; bookEl.classList.remove('wb_is-instant'); }  // flush, no transition
  updateSketchNav();
}
...
// body.style.overflow is a shared resource — a Liferay page may already have
// its own modal manager touching it, so save/restore rather than hard-reset
// (playbook §7 "scroll locks contend with any other modal manager").
...
// Full-screen overlay open/close — playbook §8 checklist. backdropEl is the
// dialog surface (role="dialog"/aria-modal in index.html); the sketchbook
// inside it is a role="group" widget, not the dialog itself.
function openOverlay() {
  if (!backdropEl) return;
  lastFocusedEl = document.activeElement;
  backdropEl.classList.add('wb_is-visible');
  backdropEl.removeAttribute('aria-hidden');
  backdropEl.inert = false;
  lockBodyScroll();
  openSketchbook();
  // Book pages are built on Scene 0, which marks the close button and page-0
  // controls reachable AFTER applyScene runs inside resetSketchbook() above —
  // safe to focus synchronously right after.
  (closeBtn || backdropEl).focus();
  rootEl.dispatchEvent(new CustomEvent('wb:open', { bubbles: true }));
}

function closeOverlay() {
  if (!backdropEl) return;
  backdropEl.classList.remove('wb_is-visible');
  backdropEl.setAttribute('aria-hidden', 'true');
  backdropEl.inert = true;
  unlockBodyScroll();
  resetSketchbook();
  if (lastFocusedEl && document.contains(lastFocusedEl)) lastFocusedEl.focus();
  rootEl.dispatchEvent(new CustomEvent('wb:close', { bubbles: true }));
}

// Close button: dismiss the delivery backdrop and reset the book.
// (Re-opening isn't wired beyond this button and the public API below —
// delivery latches once per session in this prototype.)
const closeBtn = document.querySelector('.wb_close-btn');
if (closeBtn) {
  closeBtn.addEventListener('click', closeOverlay);
}

// Public API for the host page — playbook §8. open() re-triggers delivery
// directly (same path as the ?book=1 dev shortcut below); close()/isOpen()
// mirror the close button; destroy() removes the DOM and the two listeners
// this file names (keydown, resize). It does NOT remove the mousemove/wheel
// listeners below (still anonymous) or stop the rAF loop — a known, accepted
// gap for a prototype-derived component; document before relying on it for
// a page that repeatedly creates/destroys the fragment without a full reload.
window.WelcomeBabyExperience = {
  open: openOverlay,
  close: closeOverlay,
  isOpen: () => !!backdropEl && backdropEl.classList.contains('wb_is-visible'),
  destroy: () => {
...
function initPersonaSwitcher() {
  ...
  panel.className = 'wb_persona-switcher';
  panel.innerHTML = '<p class="wb_persona-switcher__label">Preview persona</p>';
  ...
  templates.forEach((t) => {
    ...
    btn.type = 'button';
    btn.className = 'wb_persona-switcher__btn' + (isCurrent ? ' wb_persona-switcher__btn--active' : '');
...
function openBookDirect() {
  ...
  rootEl.classList.add('wb_is-delivered');
  openOverlay();
}
...
function loop(now) {
  ...
  if (!deliveryTriggered &&
    ...
    rootEl.classList.add('wb_is-delivered');
    openOverlay();   // reveal the flip book on the first spread
  }
